...
export class WahlkreisComponent implements OnInit, OnDestroy {
        ...
        callbacks: {
          label: (item: any) => {
            ...
            if (item.datasetIndex == 0 && this.erststimmenergebnissePrev && this.erststimmenergebnissePrev.length > 0) {
              const diff = 100 *
                (this.erststimmenergebnisse[item.index].rel_stimmen -
                  this.erststimmenergebnissePrev[item.index].rel_stimmen);
              diffString = `(${diff > 0 ? "+" : ""}${diff.toFixed(2)}%) `;
            }
            const abs = item.datasetIndex == 0 ?
              this.erststimmenergebnisse[item.index].abs_stimmen :
              this.erststimmenergebnissePrev[item.index].abs_stimmen;
  ...
  private populateBarChartData(data: Array<WahlkreisParteiErgebnis>,
                               config: any,
                               index: number,
